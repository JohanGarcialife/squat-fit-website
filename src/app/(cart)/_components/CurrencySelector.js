...
export default function CurrencySelector({ currency, setCurrency, currencies }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      {/* La conversión es solo orientativa: Stripe siempre cobra en euros */}
      {currency !== 'EUR' && (
        <p className="text-indigo-400 text-xs leading-snug text-center max-w-[240px]">
          El cobro se realiza en euros (EUR); el importe en {currency} es orientativo.
        </p>
      )}
    </div>
  );
