  ...
  return (
    <div className='px-6 md:px-24 flex flex-col md:flex-row justify-center items-center py-12 md:py-24 gap-10 md:gap-0'>
        ...
        <div className='flex items-center gap-4'>
            ...
            <p className='text-primary font-bold tracking-[0.2em] text-base sm:text-xl uppercase whitespace-nowrap text-center'>Biblioteca de recetas</p>
            <span className='w-8 sm:w-20 h-[2px] bg-primary rounded-full'></span>
        </div>

        <h2 className='font-bold text-secondary text-center md:text-start text-4xl sm:text-6xl md:text-6xl lg:text-6xl my-6 md:my-8 leading-tight'>La Cocina Squad Fit</h2>
        ...
        <div className='flex flex-row items-start justify-center md:justify-start gap-6 sm:gap-10 w-full'>
          <div className='text-center max-w-[110px] sm:max-w-[150px]'>
            <CountUp value={155} step={5} format={(v) => `+${Math.round(v)}`} className='text-secondary font-bold text-3xl sm:text-4xl' />
          ...
          <div className='text-center max-w-[110px] sm:max-w-[150px]'>
            ...
            <p className='text-secondary text-sm sm:text-base'>Con macros incluidos</p>
          </div>
        </div>
      </div>

        <div className="md:w-1/2 w-full flex justify-center">
          <Image src="/cocinaHero.png" width={700} height={700} alt='Cooking book' priority className='w-full max-w-[420px] md:max-w-[480px] lg:max-w-[520px] h-auto' />
        </div>
    </div>
  )
}
